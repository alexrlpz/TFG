...
for kf, (train_index, test_index) in enumerate(kfold.split(features_norm)):
    ...
    total_excitation.append(excitation)
    total_inhibition.append(inhibition)

total_excitation = np.array(total_excitation)
total_inhibition = np.array(total_inhibition)

excitation_ratio = np.mean(total_excitation, axis=0)
inhibition_ratio = np.mean(total_inhibition, axis=0)

print("\n Average E/I_exc: " + str(excitation_ratio))
print("\n Average E/I_inh: " + str(inhibition_ratio))
fig = plt.figure(figsize=[6,5], dpi=150)
ax1 = fig.add_axes([0.15,0.15,0.65,0.65])
plt.ylim(0, 0.15)
plt.plot(mouses_data['ages'], excitation_ratio, label='E/I_exc')
plt.plot(mouses_data['ages'], inhibition_ratio, label='E/I_inh')
